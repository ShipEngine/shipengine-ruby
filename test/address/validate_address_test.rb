...
describe "Validate Address" do
  ...
  it "Should successfully validate an address" do
    ...
    success_request = client.validate_addresses(params)
  ...
  it "should serialize and coerce the address fields from the server into a ruby object with the correct shape" do
    ...
    assert_nil(response[0].matched_address)
  end
  ...
  it "should return name and company name and phone (if available)" do
    ...
    response = client.validate_addresses(params)
    assert_equal("77345517190", response[0].original_address.phone)
    assert_equal("Shipmate", response[0].original_address.company_name)
